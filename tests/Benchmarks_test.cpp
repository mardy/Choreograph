...
public:
    Timer(bool start_on_creation = false) : Catch::Timer() {
...
void printTiming(const std::string &text, double milliseconds,
...
TEST_CASE("Sequence Manipulation Timing") {
    ...
    printTiming("Creating Medium Sequence rhs Assignment",
                create_auto_medium.getSeconds() * 1000);

    Timer create_huge(true);
    Sequence<float> huge_sequence(0.0f);
    for (int i = 0; i < 1.0e6; i += 1) {
        huge_sequence.then<RampTo>(cos(i * 0.1) * (i - 1.0e3), 1.0f,
                                   EaseInOutCubic());
    }
    create_huge.stop();
...
TEST_CASE("Choreograph Timeline Basic Performance") {
    ...
    for (auto &target : targets) {
        choreograph_timeline.apply(&target)
            .then<RampTo>(vec2(10.0f), 1.0f)
            .startFn([] {})
            .updateFn(
                [&m = *target.inputPtr()] { m.getCurrentValue() + vec2(1); })
            .finishFn([] {});
    }
    create_in_place.stop();
    printTiming("Creating Motions with Callbacks in place",
                create_in_place.getSeconds() * 1000);
    ...
    printTiming("60 Motion Steps (1sec at 60Hz)",
                step_created_in_place.getSeconds() * 1000);
    ...
    printTiming("Step to remove disconnected Motions",
                step_disconnected.getSeconds() * 1000);

    Timer create_copy(true);
    Sequence<vec2> sequence(vec2(1.0f));
    sequence.then<RampTo>(vec2(5.0f), 1.0f)
        .then<RampTo>(vec2(10.0f, 6.0f), 0.5f);
    for (auto &target : targets) {
        choreograph_timeline.apply(&target, sequence)
            .startFn([] {})
            .updateFn(
                [&m = *target.inputPtr()] { m.getCurrentValue() + vec2(1); })
            .finishFn([] {});
    ...
    create_copy.stop();
    printTiming("Creating Motions from existing Sequence",
                create_copy.getSeconds() * 1000);
    ...
    printTiming("60 Motion Steps (1sec at 60Hz)",
                step_copy_created.getSeconds() * 1000);
}
...
TEST_CASE("Comparative Performance with cinder::Timeline") {
    ...
    SECTION("Tween many targets") {
        ...
        for (int step = 0; step < iterations; step += 1) {
            ...
            for (int i = 0; i < tween_count; ++i) {
                cinder_timeline
                    ->apply(&cinder_targets[i], vec2(i * 5, i * 20), 2.0f)
                    .delay(1.0f);
            }
            ci_create.stop();
#endif
        ...
        printTiming("Create Performance (Choreograph / Cinder)",
                    (ch_create_avg / ci_create_avg), "");
        printTiming("Step Performance (Choreograph / Cinder)",
                    (ch_step_avg / ci_step_avg), "");
#endif
    }
    ...
    SECTION("Tween one target many times") {
        ...
        for (int j = 0; j < iterations; j += 1) {
            ...
            Timer ch_create(true);
            for (int i = 0; i < tween_count; ++i) {
                choreograph_timeline.append(&target)
                    .then<Hold>(vec2(0), 1.0f)
                    .then<RampTo>(vec2(i * 5, i * 20), 2.0f);
            }
            ch_create.stop();
            ...
            for (int i = 0; i < tween_count; ++i) {
                cinder_timeline
                    ->appendTo(&cinder_target, vec2(i * 5, i * 20), 2.0f)
                    .delay(1.0f);
            }
            ci_create.stop();
#endif
