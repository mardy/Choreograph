...
TEST_CASE("Sequences") {
    ...
    SECTION("Sequence duration is the sum of all of its component phrase's "
            "durations.") {
    ...
    SECTION("Sequence values within duration are interpolated by the "
            "appropriate Phrase.") {
    ...
    SECTION("Sequence values outside duration are clamped to begin and end "
            "values.") {
        REQUIRE(sequence.getValue(-std::numeric_limits<float>::max()) ==
                sequence.getStartValue());
        REQUIRE(sequence.getValue(std::numeric_limits<float>::max()) ==
                sequence.getEndValue());
    }
    ...
    SECTION("Looped sequence values are equivalent.") {
        ...
        REQUIRE((wrapTime(10 * sequence.getDuration() + offset,
                          sequence.getDuration()) -
                 offset) < epsilon);
        REQUIRE((sequence.getValueWrapped(sequence.getDuration() + offset) -
                 sequence.getValue(offset)) < epsilon);
        REQUIRE(
            (sequence.getValueWrapped((2 * sequence.getDuration()) + offset) -
             sequence.getValue(offset)) < epsilon);
        REQUIRE(
            (sequence.getValueWrapped((20 * sequence.getDuration()) + offset) -
             sequence.getValue(offset)) < epsilon);
    }

    SECTION("Sequences are composable.") {
        // The code generated by VS2013 differs a bit from Clang, so
        // we explicitly create the sequence phrase first to guarantee what kind
        // of sequence we get.
...
TEST_CASE("Slicing Time") {
    SECTION(
        "Clip Phrases retime existing phrases and clamp their end values.") {
    ...
    SECTION("Motions can slice their Sequence.") {
        ...
        SECTION("When sliced, the Motion's time is adjusted to be fixed "
                "relative to its Sequence.") {
            motion.jumpTo(1.0f);
            motion.sliceSequence(0.5f, 1.5f);
        ...
        SECTION("Cut In slices the Sequence so it ends in the specified amount "
                "of time.") {
            motion.cutIn(2.0f);
        ...
        SECTION(
            "Cut In also removes Phrases prior to the current Motion time.") {
            motion.jumpTo(1.0f);
            motion.cutIn(2.0f);
        ...
        SECTION("Cut In will extend the Sequence if the cut time is past the "
                "end of the Sequence.") {
            motion.jumpTo(2.5f);
            motion.cutIn(2.0f);
    ...
    SECTION("Timeline MotionOptions expose trim methods.") {
        ...
        SECTION("Cut In trims the sequence relative to the motion's current "
                "time.") {
            timeline.step(0.5f);
            float v1 = target();
            options.cutIn(0.5f);
