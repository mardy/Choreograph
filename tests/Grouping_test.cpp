...
TEST_CASE("Motion Groups") {
    ...
    SECTION("Timelines are composable.") {
        ...
        int receiver = 0;
        t2->apply(&target).then<RampTo>(50, 1.0f);
        t2->cue([&receiver] { receiver = 100; }, 0.4f);
    ...
    SECTION("Motion Group member callbacks are still called.") {
        ...
        group_timeline.apply(&target)
            .then<RampTo>(10, 1.0f)
            .startFn([&start_count]() { start_count += 1; })
            .updateFn([&update_count]() { update_count += 1; })
            .finishFn([&finish_count]() { finish_count += 1; });

        SECTION("Sanity Check") { REQUIRE(group->getDuration() == 1.0f); }

        SECTION("Looping the group still calls child callbacks.") {
            group->setFinishFn(
                [&group_timeline]() { group_timeline.resetTime(); });
        ...
        SECTION("Ping-pong looping a group works, too.") {
            group->setFinishFn([&group_timeline]() {
                // MotionGroup overrides customSetPlaybackSpeed to inform
                // children.
                group_timeline.setPlaybackSpeed(
                    -1 * group_timeline.getPlaybackSpeed());
            ...
            // We only pass the start and finish of our motion going forward
            // twice. We also pass them each going backward, but that doesn't
            // trigger our functions.
