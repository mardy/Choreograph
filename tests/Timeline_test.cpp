...
TEST_CASE("Timeline") {
    ...
    SECTION(
        "Convenience methods create equivalent timeline to explicit methods") {
        Output<float> a, b;
        timeline.apply(&a).set(0.0f).rampTo(5.0f, 1.0, EaseOutQuad());

        timeline.apply(&b).set(0.0f).then<RampTo>(5.0f, 1.0, EaseOutQuad());
...
TEST_CASE("Callbacks") {
    ...
    SECTION(
        "Functions can be cued by motion events: start, update, and finish.") {
        ...
        options.startFn([&startCalled]() { startCalled = true; })
            .updateFn([&updateTarget, &target, &updateCount]() {
                updateTarget = target() / 2.0f;
                updateCount++;
            })
    ...
    SECTION("It is safe to add and cancel motions from Cues and Motion "
            "callbacks.") {
        Output<float> t2 = 1.0f;

        SECTION("Add Motion from Motion callback.") {
            options.startFn([&]() { timeline.apply(&t2, sequence); });
        ...
        SECTION("Cancel Motion from Motion callback.") {
            // Play t2 motion twice as fast as previous, disconnect target on
            // finish.
            timeline.apply(&t2, sequence)
                .playbackSpeed(2.0f)
                .finishFn([&target]() { target.disconnect(); });
        ...
        SECTION("Change Motion from Cue.") {
            timeline.cue([&] { timeline.append(&target).then(sequence); },
                         0.5f);
