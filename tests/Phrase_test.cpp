...
TEST_CASE("Phrases") {
    ...
    SECTION("Procedural phrases return values from functional procedures.") {
        auto proc = makeProcedure<float>(
            1.0f, [](Time t, Time duration) { return sin(t * PI) * 10.0f; });
    ...
    SECTION("Accumulate phrases mix the output of multiple phrases into one.") {
        ...
        auto decumulate = makeAccumulator<float>(
            0.0f, ramp, other, [](float a, float b) { return a - b; });
    ...
    SECTION("Ramps and Mix Phrases receive optional interpolation functions to "
        ...
        auto lerp_obj = [](const Obj &lhs, const Obj &rhs, float mix) {
            return Obj{ch::lerpT(lhs.x, rhs.x, mix),
                       ch::lerpT(lhs.y, rhs.y, mix), lhs.name};
        };
        ...
        REQUIRE(mix_ramps->getValue(0.5f).y ==
                ((550.0f * 0.5f) + (55.0f * 0.5f)));
    }
}
