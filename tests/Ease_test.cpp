...
TEST_CASE("Separate component interpolation", "[sequence]") {
    ...
    SECTION("Compare 2-Component Values") {
        ...
        REQUIRE(sequence.getValue(2.0f).x ==
                sequence.getValue(2.0f).y); // past the end
        REQUIRE(sequence.getValue(0.5f).x != sequence.getValue(0.5f).y);
    }

    SECTION("Compare 3-Component Values") {
        Sequence<vec3> sequence(vec3(1));
        sequence.then<RampTo3>(vec3(10.0f), 1.0f, EaseOutQuad(), EaseInQuad(),
                               EaseInOutQuad());
    ...
    SECTION("Compare 4-Component Values") {
        ...
        sequence.then<RampTo4>(vec4(10.0f), 1.0f, EaseOutQuad(), EaseInAtan(),
    ...
    SECTION("Over- and Under-fill Separate Easings") {
        ...
        // The real test here is that we don't crash when creating or using this
        // sequence.
        sequence.then<RampTo3>(vec3(1.0f), 1.0f, EaseInOutQuad())
            .then<RampTo3>(vec3(5.0f), 1.0f, EaseInQuad(), EaseNone(),
                           EaseInAtan(), EaseInBack());
